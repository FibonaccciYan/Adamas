...
void bench_flashinfer_append_prefill(nvbench::state& state) {
	...
	thrust::device_vector<T> chunk_data(num_chunks * num_kv_heads * page_size * head_dim/8);
	...
	thrust::device_vector<T> h(batch_size * num_kv_heads * head_dim * seqlen);

	state.add_global_memory_reads<uint8_t>(vec_bytes(k) + vec_bytes(v) + vec_bytes(h) + vec_bytes(kv_indptr) +
											   vec_bytes(kv_indices) + vec_bytes(append_indptr) +
											   vec_bytes(chunk_data) + vec_bytes(chunk_indptr) +
											   vec_bytes(chunk_indices),
										   "Read");
	state.add_global_memory_writes<uint8_t>(vec_bytes(k) + vec_bytes(v) + vec_bytes(h) / 8 + vec_bytes(chunk_data),
											"Write");
...
void bench_flashinfer_append_decode(nvbench::state& state) {
	...
	std::shuffle(chunk_indicies_host.begin(),
				 ...
				 std::mt19937(std::random_device()()));
	int32_t last_chunk_idx = chunk_indicies_host.back();
	std::vector<T> chunk_data_host(num_chunks * num_kv_heads * page_size * head_dim/8);

	std::vector<T> query(1 * num_qo_heads * head_dim);
	...
	thrust::device_vector<T> output(1 * num_qo_heads * head_dim/8);
	thrust::device_vector<T> o(output);

	state.add_global_memory_reads<uint8_t>(
		vec_bytes(q) + vec_bytes(k) + vec_bytes(v) + vec_bytes(kv_page_data) +
			vec_bytes(page_indptr) + vec_bytes(page_indices) + vec_bytes(chunk_data) +
			vec_bytes(chunk_indptr) + vec_bytes(chunk_indices),
		"Read");
	state.add_global_memory_writes<uint8_t>(
		vec_bytes(k) + vec_bytes(v) + vec_bytes(o) + num_kv_heads * head_dim / 8 * sizeof(T),
		"Write");

	// call kernel
	state.exec([&](nvbench::launch&) {
		cudaError_t status = AppendPagedKVCacheDecodeFused<PageStorage::kIndices, kv_layout, T, int32_t>(
			paged_kv,
			paged_hadamard,
...
void bench_flashinfer_qk_apply_rope(nvbench::state& state) {
	...
	std::vector<T> key(seqlen * num_kv_heads * head_dim);
	std::vector<T> query(seqlen * num_qo_heads * head_dim);
	// Move to GPU
	thrust::device_vector<T> k(key);
	thrust::device_vector<T> q(query);
...
#define BENCH_FLASHINFER_APPEND_DECODE(dtype)                                               \
	...
	NVBENCH_BENCH(bench_flashinfer_append_decode_##dtype##_)                                \
		.set_name("bench_flashinfer_append_decode_fused_" STR(dtype))                       \
		.add_int64_axis("seqlen", {4096, 8192, 16384, 32768, 65536})                        \
		.add_int64_axis("page_size", {1})                                                  \
...
#define BENCH_FLASHINFER_QK_APPLY_ROPE(dtype)                                               \
	...
	NVBENCH_BENCH(bench_flashinfer_qk_apply_rope_##dtype##_)                                \
		...
		.add_int64_axis("seqlen", {1, 4, 8, 16, 4096, 8192, 16384})                        \
		.add_int64_axis("num_qo_heads", {32})                                              \
		.add_int64_axis("num_kv_heads", {8, 32})
