...
void bench_flashinfer_append_decode(nvbench::state& state) {
	...
	std::vector<T> hadamard(2 * num_kv_heads * head_dim);
	thrust::device_vector<T> chunk_data(chunk_data_host);
	thrust::device_vector<int32_t> chunk_indptr(chunk_indptr_host);
	thrust::device_vector<int32_t> chunk_indices(chunk_indicies_host);
	thrust::device_vector<T> h(key);
	paged_kv_t<PageStorage::kIndices, kv_layout, T, int32_t> paged_hadamard(
		num_kv_heads,
		page_size,
		head_dim/8,
	...
	thrust::device_vector<T> output(1 * num_kv_heads * head_dim/8);
	thrust::device_vector<T> o(output);

	state.add_global_memory_reads<uint8_t>(
		2 * vec_bytes(k) + 2 * vec_bytes(v) + vec_bytes(h) + vec_bytes(page_indptr), "Read"); 
	state.add_global_memory_writes<uint8_t>(2 * vec_bytes(k) + 2 * vec_bytes(v) + vec_bytes(h) / 8 / 2 + vec_bytes(o), "Write");
...
void bench_flashinfer_qk_apply_rope(nvbench::state& state) {
	...
	std::vector<T> query(seqlen * num_kv_heads * head_dim);
	// Move to GPU
	thrust::device_vector<T> k(key);
