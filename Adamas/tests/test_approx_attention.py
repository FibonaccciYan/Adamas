...
def test_approx_attention_correctness(dtype_str, qo_len, kv_len, token_budget):
    ...
    k_prefill_code = torch.bucketize(hadamard_transform(k_prefill), testController.thresholds, out_int32=True)
    ...
    k_decode_code = torch.bucketize(hadamard_transform(k_decode),   testController.thresholds, out_int32=True)
