...
void append_kv_cache_decode(torch::Tensor k,
	...
	CHECK_INPUT(h_q); // [seq_len, num_qo_heads, head_dim]
	CHECK_INPUT(h_k); // [seq_len, num_kv_heads, head_dim]
	...
	CHECK_EQ(h_q.size(0), 1); // decode
	CHECK_EQ(h_k.size(0), 1); // decode
	...
	size_t num_qo_heads = h_q.size(1);
	size_t num_kv_heads = k.size(1);
	size_t head_dim = k.size(2);

	CHECK_EQ(h_q.size(1) % k.size(1), 0); // num_qo_heads should be divisible by num_kv_heads
	CHECK_EQ(v.size(1), num_kv_heads);
	CHECK_EQ(h_k.size(1), num_kv_heads);
	CHECK_EQ(h_q.size(2), head_dim);
	CHECK_EQ(h_k.size(2), head_dim);
		...
		SWITCH_LAYOUT(kv_layout, KV_LAYOUT, {
			...
			cudaError_t status =
				AppendPagedKVCacheDecode<PageStorage::kIndices, KV_LAYOUT, c_type, int32_t>(
					...
					static_cast<c_type*>(h_q.data_ptr()),
					static_cast<c_type*>(h_k.data_ptr()),
